...
if (!defined('__TYPECHO_ROOT_DIR__')) exit(1);
	...
	if ($delete) {
		...
		if (Helper::options()->JFriendsStatusEmail == 'on') {
			foreach ($friends as $key => $value) {
				if ($value['status'] && !empty($value['email']) && preg_match($email_preg, $value['email'])) {
					if (joe_send_mail('您的友情链接已被删除', '', [
						'友链标题' => $value['title'],
						'友链网址' => $value['url'],
						'友链描述' => $value['description'],
					], $value['email'])) $send_mail++;
				}
			}
		}
		$notice->set('友链 [' . implode(',', $id) . '] 已删除' . ($send_mail ? '并向对方发送邮箱通知' : ''), 'success');
		$response->goBack();
	} else {
	...
	if ($update) {
		...
		if (Helper::options()->JFriendsStatusEmail == 'on') {
			...
			foreach ($friends as $value) {
				if (!empty($value['email']) && preg_match($email_preg, $value['email'])) {
					if (joe_send_mail('您的友情链接已通过审核', '', [
						'友链标题' => $value['title'],
						'友链网址' => $value['url'],
						'友链描述' => $value['description'],
					], $value['email'])) $send_mail++;
		...
		$notice->set('友链 [' . implode(',', $id) . '] 已启用' . ($send_mail ? '并向对方发送邮箱通知' : ''), 'success');
		$response->goBack();
	} else {
...
if ($action == 'disable') {
	...
	if (!is_array($id)) $id = [$id];
	$update = Db::name('friends')->whereIn('id', $id)->update(['status' => 0]);
	if ($update) {
		$send_mail = 0;
		if (Helper::options()->JFriendsStatusEmail == 'on') {
			$friends = getFriends($id);
		...
		$notice->set('友链 [' . implode(',', $id) . '] 已禁用' . ($send_mail ? '并向对方发送邮箱通知' : ''), 'success');
		$response->goBack();
	} else {
