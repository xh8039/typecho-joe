...
<div class="author-header mb20 radius8 main-shadow main-bg full-widget-sm">
	<div class="page-cover">
		...
		if (joe_url_is_video($user_cover)) {
			?><div class="user-cover fit-cover" style="padding-bottom: 0;"><video width="100%" src="<?= $user_cover ?>" autoplay loop muted preload="none"></video></div><?php
		} else {
			?><img referrerpolicy="no-referrer" rel="noreferrer" class="lazyload fit-cover user-cover user-cover-id-<?= $uid ?>" src="<?= joe_theme_url('assets/img/thumbnail-lg.svg') ?>" data-src="<?= $user_cover ?: joe_theme_url('assets/img/user_t.jpg') ?>" alt="用户封面"><?php
		}
		?>
		...
		<div class="abs-center right-bottom padding-6 cover-btns">
			<span class="dropup pull-right">
				...
				<ul class="dropdown-menu">
					<li><a data-class="modal-mini" mobile-bottom="true" data-height="330" data-remote="<?= joe_api_url('user_details_data_modal',['id'=>$author->uid]) ?>" class="user-details-link " href="javascript:;" data-toggle="RefreshModal"><svg class="mr6" aria-hidden="true"><use xlink:href="#icon-user"></use></svg>更多资料</a></li>
					<li><a search-user="1" search-trem="null" search-placeholder="在用户[<?= $author->screenName ?>]中搜索内容" class="main-search-btn" href="javascript:;"><svg class="mr6" aria-hidden="true"><use xlink:href="#icon-search"></use></svg>搜索内容</a></li>
					<?= $author->uid === $this->user->uid ? '<li><a mobile-bottom="true" data-height="330" data-remote="'.joe_api_url('user_cover_set_modal').'" class="avatar-set-link " href="javascript:;" data-toggle="RefreshModal"><i class="fa fa-camera mr6" aria-hidden="true"></i>修改封面</a></li>' : '' ?>
				</ul>
			</span>
		</div>
	</div>
	<div class="header-content">
		<div class="flex header-info relative hh">
			<div class="flex0 header-avatar"><span class="avatar-img"><img alt="<?= $author->screenName ?>的头像-<?= $this->options->title ?>" src="<?= joe_avatar_lazyload_url() ?>" data-src="<?= joe_avatar_url_by_mail($author->mail) ?>" class="alone-imgbox-img lazyload avatar avatar-id-1"></span></div>
			<div class="flex1">
			...
			<div class="header-btns flex0 flex ac">
				...
				if ($this->user->hasLogin()) {
					if ($this->user->uid === $author->uid) {
						?>
						<a rel="nofollow" href="<?= joe_build_url('user/index') ?>" class="but c-blue ml10 pw-1em radius"><svg class="icon" aria-hidden="true"><use xlink:href="#icon-user"></use></svg>用户中心</a>
					...
					?>
					<a href="javascript:;" class="but jb-pink ml10 pw-1em signin-loader" data-pid="1"><count><i class="fa fa-heart-o mr3" aria-hidden="true"></i>关注</count></a>
					<a class="signin-loader but jb-blue ml10 pw-1em" href="javascript:;"><svg class="icon" aria-hidden="true"><use xlink:href="#icon-private"></use></svg>私信</a>
					<?php
				}
				?>
			</div>
