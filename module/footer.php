	...
	<div class="container-fluid container-footer">
		...
		<ul class="list-inline">
			<li class="hidden-xs" style="max-width: 300px;">
				<p><a class="footer-logo" href="/" title="<?php $this->options->title() ?>"><img referrerpolicy="no-referrer" rel="noreferrer" src="<?= joe_theme_url('assets/img/thumbnail-sm.svg', null) ?>" data-src="<?= joe_logo_url() ?>" switch-src="<?= joe_logo_url('dark') ?>" alt="<?= $this->options->title ?>" class="lazyload" style="height: 40px;"></a></p>
			...
			<?php
			$JFooter_Right_Image = joe_option_multi($this->options->JFooter_Right_Image);
...
<div class="float-right round position-bottom <?= joe_is_mobile() ? 'filter' : '' ?> scroll-down-hide">
	...
	if (joe_is_pc()) {
		?>
		<a rel="nofollow" class="newadd-btns float-btn add-btn btn-newadd" href="<?= $this->user->hasLogin() ? joe_relative_url($this->options->adminUrl) . 'write-post.php' : joe_user_auth_url('login') ?>"><i class="fa fa-pencil" aria-hidden="true"></i></a>
	...
	<!-- <a class="float-btn service-wechat hover-show nowave" title="扫码添加微信" href="javascript:;">
		...
		<div class="hover-show-con dropdown-menu">
			<img class="radius4 relative" width="100%" class="lazyload" src="<?= joe_theme_url('assets/img/thumbnail-sm.svg', null) ?>" data-src="<?= joe_theme_url('assets/img/qrcode.png', null) ?>" alt="扫码添加微信-Joe主题">
		</div>
	</a> -->
	...
	<?php
	if (in_array($this->user->group,['administrator','editor']) && $this->is('post')) {
		echo ' <a href="javascript:;" data-toggle="modal" data-target="#modal_admin_set" title="设置页面参数" class="admin-set-page float-btn"><i class="fa fa-cog fa-spin" aria-hidden="true"></i><div class="abs-right badg c-red px12 admin-set-info" style="width:187px;">在此编辑此页面参数</div></a>';
	}
	?>
</div>
<?php
if (in_array($this->user->group,['administrator','editor']) && $this->is('post')) $this->need('module/modal/setting.php');
if (joe_is_mobile()) {
?>
	<div class="footer-tabbar">
		...
		if ($this->is('post')) {
			...
			if ($is_comment) : ?><div class="flex ac jsb virtual-input tabbar-item" fixed-input="#respond"><div class="flex flex1 ac"><img alt="<?= $this->author->screenName ?>的头像-<?php $this->options->title() ?>" src="<?= joe_avatar_lazyload_url() ?>" data-src="<?= joe_avatar_url_by_mail($this->author->mail) ?>" class="lazyload avatar avatar-id-<?= $this->author->uid ?>"><div class="text-ellipsis simulation mr10">欢迎您留下宝贵的见解！</div></div><span class="but c-blue">提交</span></div><?php endif;?>
...
?>
	<div style="width:45px;height:45px;z-index:9;position:fixed;right:0;bottom:0;cursor:pointer;">
