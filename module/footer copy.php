...
<div class="joe_action">
	...
	?>
	<?php
	if ($this->is('post') && joe_is_mobile() && $this->options->JArticle_Guide == 'on') {
	?>
	...
	<?php if ($this->user->uid == $this->authorId) : ?>
		<?php if ($this->is('post')) : ?>
			<div class="joe_action_item" data-toggle="tooltip" data-placement="left" title="编辑文章">
				<a target="_blank" href="<?= joe_root_relative_link($this->options->adminUrl); ?>write-post.php?cid=<?php echo $this->cid; ?>"><i class="fa fa-cog fa-spin"></i></a>
			</div>
		<?php elseif ($this->is('page')) : ?>
			<div class="joe_action_item" data-toggle="tooltip" data-placement="left" title="编辑页面">
				<a target="_blank" href="<?= joe_root_relative_link($this->options->adminUrl); ?>write-page.php?cid=<?php echo $this->cid; ?>"><i class="fa fa-cog fa-spin"></i></a>
			</div>
		<?php endif; ?>
	<?php endif; ?>
</div>
...
?>
		...
		<div class="container-fluid container-footer">
			...
			<ul class="list-inline <?= empty($this->options->baidu_statistics) ? null : 'flex' ?>">
				<li class="hidden-xs" style="max-width: 300px;">
					<p>
						<a class="footer-logo" href="/" title="<?php $this->options->title() ?>">
							<img referrerpolicy="no-referrer" rel="noreferrer" src="<?= joe_theme_url('assets/images/thumb/thumbnail-sm.svg', false) ?>" data-src="<?php empty($this->options->JLogo) ? $this->options->themeUrl('assets/images/logo.png') : $this->options->JLogo(); ?>" alt="<?php $this->options->title() ?>" class="lazyload light" style="height: 40px;">
							<img referrerpolicy="no-referrer" rel="noreferrer" src="<?= joe_theme_url('assets/images/thumb/thumbnail-sm.svg', false) ?>" data-src="<?php $this->options->JDarkLogo(); ?>" alt="<?php $this->options->title() ?>" class="lazyload dark" style="height: 40px;">
						</a>
					</p>
							...
							?>
								<a class="toggle-radius hover-show nowave" href="javascript:;">
									...
									<div class="hover-show-con footer-wechat-img">
										<img style="box-shadow: 0 5px 10px rgba(0,0,0,.2); border-radius:4px;" height="100" class="lazyload" src="<?= joe_theme_url('assets/images/thumb/thumbnail-sm.svg', false) ?>" data-src="<?= $this->options->JFooterContactWechatImg ?>" alt="扫一扫加微信 - <?= $this->options->title ?>">
									</div>
								</a>
							<?php
							}
				...
				if (!empty($JFooter_Right_Image)) {
					echo '<li>';
					foreach ($JFooter_Right_Image as $key => $value) {
					?>
						<div class="footer-miniimg" data-toggle="tooltip" title="<?= $value[0] ?? '' ?>">
							<p>
								<img referrerpolicy="no-referrer" rel="noreferrer" class="lazyload" src="<?= joe_theme_url('assets/images/thumb/thumbnail-sm.svg', false) ?>" data-src="<?= $value[1] ?? '' ?>" alt="<?= $value[0] ?? '' ?> - <?= $this->options->title ?>">
							</p>
	...
	if ($this->options->JFooter_Fish == 'on') {
		echo '<div id="footer_fish" style="background: var(--main-bg-color);"></div><script defer src="' . joe_theme_url('assets/plugin/FooterFish.js') . '"></script>';
	}
} else {
	?>
	<footer class="joe_footer">
		...
		<?php
		if ($this->options->JFooter_Fish == 'on') {
			echo '<div id="footer_fish"></div><script defer src="' . joe_theme_url('assets/plugin/FooterFish.js') . '"></script>';
		}
		?>
	</footer>
<?php
...
?>
	<div id="cc-myssl-seal" style="width:65px;height:65px;z-index:9;position:fixed;right:0;bottom:0;cursor:pointer;">
		<div title="TrustAsia 安全签章" id="myssl_seal" style="text-align: center">
			<img src="<?= joe_theme_url('assets/images/myssl-id.png') ?>" alt="SSL" style="width: 100%; height: 100%"></a>
		</div>
	</div>
<?php
}

if (!empty($this->options->joe_footer_tab) && joe_is_mobile()) {
	$footer_tabbar = joe_optionMulti($this->options->joe_footer_tab);
} else {
	$footer_tabbar = false;
}
if (!empty($footer_tabbar)) {
?>
	<link rel="stylesheet" href="<?= joe_theme_url('assets/css/options/footer-tabbar.css') ?>">
	<div class="footer-tabbar">
		<?php
		$admin_dir_preg_quote = preg_quote(__TYPECHO_ADMIN_DIR__, '/');
		foreach ($footer_tabbar as $value) {
			$value[1] = $value[1] ?? '';
			if (preg_match("/{$admin_dir_preg_quote}[a-z,-]+\.php/i", $value[1], $match) && !$this->user->hasLogin()) $value[1] = joe_user_auth_url('login', $match[0]);
		?>
...
<?php if ($this->options->JAside_3DTag == 'on') : ?>
	<script defer src="<?= joe_theme_url('assets/plugin/3dtag/3dtag.min.js', null); ?>"></script>
<?php endif; ?>
...
<?php if ($this->options->JCursorEffects && $this->options->JCursorEffects != 'off') : ?>
	<script defer src="<?= joe_theme_url('assets/plugin/cursor/' . $this->options->JCursorEffects, null) ?>"></script>
<?php endif; ?>
<?php
