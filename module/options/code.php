...
if (isset($_GET['joe_code'])) {
	$JCustomCodeHTML = new \Typecho\Widget\Helper\Form\Element\Hidden(
		'',
		NULL,
		NULL,
		'
	...
	$JCustomCodeHTML->setAttribute('class', 'joe_content joe_code');
	$form->addInput($JCustomCodeHTML);
}
...
$joe_sidebar_html->setAttribute('class', 'joe_content joe_code');
$joe_sidebar_html->setAttribute('data-language', 'htmlmixed');
...
$JCustomHeadEnd = new \Typecho\Widget\Helper\Form\Element\Textarea(
	'JCustomHeadEnd',
	NULL,
	'<script src="//at.alicdn.com/t/c/font_4742095_cefbwapchxj.js"></script>',
	'自定义头部HTML代码（非必填）',
...
$JArticleHeaderHTML = new \Typecho\Widget\Helper\Form\Element\Textarea(
	...
	NULL,
	'文章内容前-插入HTML（非必填）',
	htmlentities('介绍：在每篇文章顶部插入内容，可以插入广告或者文章说明等内容，位于 <article> 标签下方，文章内容上方，请注意代码规范及标签闭合') . '<br>' . 
	htmlentities('例如：<div class="wp-block-zibllblock-alert alert-dismissible fade in"><div class="alert jb-blue" data-isclose="" role="alert">Ctrl+D 收藏本站 再次访问不迷路 ~</div></div>')
);
...
$JArticleBottomHTML = new \Typecho\Widget\Helper\Form\Element\Textarea(
	...
	NULL,
	'文章内容后-插入HTMl（非必填）',
	htmlentities('介绍：在每篇文章尾部插入内容，可以插入广告或者文章说明等内容，位于 </article> 标签上方，文章内容下方，请注意代码规范及标签闭合')
);
...
$JFcodeCustomizeCode->setAttribute('class', 'joe_content joe_code');
$JFcodeCustomizeCode->setAttribute('data-language', 'htmlmixed');
...
// 	'自定义添加主题 functions.php 文件里面的PHP代码，代码开头需要加上 <\?php 声明，<\?php 中的 \ 需要在实际填写的时候删除，不懂请勿乱填'
// );
