...
if (is_array($loading_scan)) {
	foreach ($loading_scan as $value) {
		...
		if ($loading_name === 'animation') continue;
...
$joe_global_loading_animation = new \Typecho\Widget\Helper\Form\Element\Select(
	'joe_global_loading_animation',
	$loading_list,
	'',
	'全局加载动画',
	'介绍：网站全局页面加载 Loading 动画，开启后可防止使用谷歌内核的浏览器出现闪动问题，网络不好，或显示不正常请关闭！'
);
$joe_global_loading_animation->setAttribute('class', 'joe_content joe_decoration');
$form->addInput($joe_global_loading_animation->multiMode());

$joe_global_loading_animation_only_first = new \Typecho\Widget\Helper\Form\Element\Select(
	'joe_global_loading_animation_only_first',
	['off' => '关闭（默认）', 'on' => '开启'],
	'off',
	'仅首次加载动画',
	'介绍：只在用户首次进入网站时展示加载动画'
);
$joe_global_loading_animation_only_first->setAttribute('class', 'joe_content joe_decoration');
$form->addInput($joe_global_loading_animation_only_first->multiMode());
