<li class="comment byuser comment-author-<?= $comment->author ?> <?= $bypostauthor ?> <?php $comment->alt('odd alt thread-odd thread-alt', 'even thread-even'); ?> depth-1" id="comment-<?= $comment->coid ?>">
    <ul class="list-inline">
        <li class="comt-main" id="div-comment-<?= $comment->coid ?>">
            <div class="comment-header mb10">
                <div class="author-box flex ac">
                    <?php
                    echo '<a href="' . $comment->url . '"><span class="avatar-img comt-avatar"><img alt="' . $comment->author . '的头像-' . Helper::options()->title . '" src="' . joe_avatar_lazyload_url() . '" data-src="' . joe_get_avatar_by_mail($comment->mail) . '" class="lazyload avatar avatar-id-' . $comment->authorId . '"></span></a>';
                    echo '<name class="flex ac flex1"><a class="text-ellipsis font-bold" href="' . $comment->url . '">' . $comment->author . '</a>' . $comment_author_badg . '</name>';
                    echo '<a href="javascript:;" data-action="comment_like" class="action action-comment-like muted-2-color flex0 ml10" data-pid="' . $comment->coid . '"><svg class="icon mr3" aria-hidden="true"><use xlink:href="#icon-like"></use></svg><text></text><count>0</count></a>';
                    ?>
                </div>
            </div>
            <div class="comment-footer">
                ...
                <div class="comt-meta muted-2-color">
                    <span class="comt-author" title="<?= date('Y年m月d日 H:i:s', $comment->created) ?>"><?= joe_dateWord($comment->dateWord) ?></span>
                    <?php
                    echo $parent_author;
