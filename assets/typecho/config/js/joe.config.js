...
window.Joe.togglePanelDisplay = (currentTab) => {
	// 切换通知和表单面板
	const isNoticeTab = currentTab === "joe_bulletin";
	if (window.jQuery) {
	...
	$.ajax({
		...
		success: (data) => {
			...
			if (data.code == 200) {
				...
				layer.confirm(data.message, { btn: ['下载更新包', '关闭'] }, () => { Joe.openLinkInNewTab(file); });
			}
		},
		error: () => {
			layer.close(loading);
...
window.Joe.updateConfirm = (version, file) => {
	layer.confirm(`发现新版本V${version}，老版本即刻起停止维护，建议您立即更新！`, { btn: ['立即更新', '暂不更新'] }, () => {
		Joe.themeUpdate(file);
	}, () => {
...
document.addEventListener("DOMContentLoaded", function () {

	{
		...
		if ($('[data-current="joe_code"]').hasClass('active')) sessionStorage.setItem("joe_config_current", 'joe_bulletin');
	}
